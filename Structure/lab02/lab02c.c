...
void myprint(int ** matrix, int *cols, int row); //#Solution
...
int main(void) {
	...
	cols = malloc(sizeof(int)*row); // 2. dynamically allocate a 1d int array of size row //#Solution
	...
	for (i = 0; i < row; i++) {
		...
		scanf("%d", &column);
		cols[i] = column;
		//matrix[i] =  // 3. allocate a 1d int array of size column that inializes the array to zeros //#Solution
...
void myprint(int **matrix, int *cols, int row){
	for(int i = 0; i < row; i++){
		for(int j = 0; j < cols[i]; j++){
			printf("%d ",*(matrix[i] + j));
		}
